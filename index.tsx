...
【生成指标】：
你必须一次性生成且仅生成 4 篇完全独立的深度推文。严禁只写 1 篇，严禁合并。文章内容最好不要带有时间属性。

【Nutty 深度内容参考模版】：
每一篇文章必须包含以下结构：
1. [标题]：直击痛点，如《如何彻底防范...》、《2026年最新趋势...》15字以内
2. [背景摘要]：100字以内的引言。
3. [核心要点]：使用 1. 2. 3. 序号清晰陈述事实或步骤。
4. [深度分析/对策]：提供专业建议或避雷指南，但少用过于专业的名词。
...
---POST_DIVIDER---`;
      ...
      const userPrompt = `
        主题: "${topic}"
        要求: "${context}"
        请立即通过深度思考并分析，按上述 Nutty 深度内容标准，且去除[标题]、[背景摘要][核心要点][深度分析/对策]字样，生成 4 篇风格和切入点迥异的推文600字左右。
      `;
